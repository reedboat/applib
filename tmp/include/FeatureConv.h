...
class FeatureConv{
	...
	public:
		...
		bool IsFeatureWord(string & word);//bloom filter for fast finding
	...
	private:
		...
		SuperString m_SS;
		vector<string> m_FeatureVec;//for bloom filer
		bloom_filter  * m_BloomFilter   ;//bloom filter for fast finding
};
#endif
