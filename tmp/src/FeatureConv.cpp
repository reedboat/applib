...
bool FeatureConv::Init(string & featureFile,bool bSemantic)
{
	...
	int wordSize=m_FeatureVec.size();                                            
	const double desired_probability_of_false_positive = 1.0 /wordSize;           
	unsigned int random_seed = 111;                                               
	                                                                                       
	m_BloomFilter=new bloom_filter(wordSize,desired_probability_of_false_positive,random_seed);  
	m_BloomFilter->insert(m_FeatureVec.begin(),m_FeatureVec.end()); 
...
void FeatureConv::CombineWord(vector<string>  & wordList,map<string,int> & wordTf)
{
	...
	{
		//firstly,discard the NON-Feature words
		if(IsFeatureWord((*it))==true)
		{
			//cout<<"find feature word="<<(*it)<<endl;
...
bool FeatureConv::IsFeatureWord(string & word)
{
	return m_BloomFilter->contains(word);
